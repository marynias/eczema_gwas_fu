#! /usr/bin/env python
#Generate GWAS input table for coloc, if we only have p-values available.
...
ap.add_argument('--eaf',required=True,type=int,help='Column with EAF (effect allele frequency) information in the --tab file')   
...
with open (proces) as proces_h:
	for line in proces_h:
		...
		filename = lines[0] + "_" + str(interval) + "kbp.gwas"
		files_dict.setdefault(filename, open(filename,'w')) 
		files_dict[filename].write(header + "\n")


def evaluate_snp(my_id,my_chrom,my_pos,my_line):
	for snp_index in my_loci.keys():
		...
		if snp_chrom == my_chrom:
			...
			if (my_pos >= lower_bound and my_pos <= upper_bound):
				filename =  snp_index + "_" + str(interval) + "kbp.gwas"
				#Change EAF to MAF (minor allele frequency)
				if float(lines[eaf-1]) > 0.5:
					current_eaf = lines[eaf-1]
					lines[eaf-1] = str(1 - float(current_eaf))
				files_dict[filename].write("\t".join(lines) + "\n")
			else:
				continue
		else:
			continue
...
for line in file_stream:
	...
	my_id = lines[ident-1]
	evaluate_snp(my_id,my_chrom,my_pos,lines)
