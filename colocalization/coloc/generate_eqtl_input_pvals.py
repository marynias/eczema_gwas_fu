...
with open (proces) as proces_h:
	for line in proces_h:
		...
		files_dict[filename].write("rsid" + "\t" + "chrom" + "\t" + "pos" + "\t" + "pval" + "\n")
...
def evaluate_snp(my_id,my_chrom,my_pos,my_pval):
	for snp_index in my_loci.keys():
		...
		if snp_chrom == my_chrom:
			...
			if (my_pos >= lower_bound and my_pos <= upper_bound):
				...
				files_dict[filename].write(my_id + "\t" + str(my_chrom) + "\t" + str(my_pos) + "\t" + str(my_pval) + "\n")
			else:
				continue
		else:
			continue
...
for line in file_stream:
	...
	evaluate_snp(my_id,my_chrom,my_pos,my_pval)
