...
#Prepare TWAS-formated input from TwinsUK - skin.
...
plink --gen $twinsuk/skin/${snp}_3Mbp_all.gen --sample $twinsuk/skin/${snp}_3Mbp_all.sample --oxford-single-chr $chrom --make-bed --out ${snp}_3Mbp_all
plink --gen $twinsuk/skin/${snp}_3Mbp_eczema.gen --sample $twinsuk/skin/${snp}_3Mbp_eczema.sample --oxford-single-chr $chrom  --make-bed --out ${snp}_3Mbp_eczema
plink --gen $twinsuk/skin/${snp}_3Mbp_noneczema.gen --sample $twinsuk/skin/${snp}_3Mbp_noneczema.sample --oxford-single-chr $chrom --make-bed --out ${snp}_3Mbp_noneczema
done

#Add gene expression to each gene.
for snp in rs61813875 rs10791824 rs6419573 rs12188917 rs2212434 rs2918307 rs4809219 rs4713555 rs2041733 rs2944542 rs145809981 rs6827756 rs12730935 rs4312054 rs1249910 rs2592555 rs7127307 rs2038255 rs6602364 rs6473227 rs7512552 rs112111458 rs7625909 rs4643526 rs10199605 rs10214237 rs12951971 rs1057258 rs6872156 rs7016497 rs2905493 rs1799986 rs2227483 rs7146581 rs11657987 rs77714197 rs3917265 rs13152362 rs4705962 rs2218565 rs7512552 rs12730935 rs12295535 rs11923593 rs2143950 rs17881320
do
python $scripts/generate_twas_input_twinsuk.py $twinsuk/skin/ensemble_ids_unique_ensembl_names \
$twinsuk/skin/${snp}_3Mbp_snps_headers_all.rpkm \
${snp}_3Mbp_all.fam \
${snp}_3Mbp_all
python $scripts/generate_twas_input_twinsuk.py $twinsuk/skin/ensemble_ids_unique_ensembl_names \
$twinsuk/skin/${snp}_3Mbp_snps_headers_eczema.rpkm \
${snp}_3Mbp_eczema.fam \
${snp}_3Mbp_eczema
python $scripts/generate_twas_input_twinsuk.py $twinsuk/skin/ensemble_ids_unique_ensembl_names \
$twinsuk/skin/${snp}_3Mbp_snps_headers_noneczema.rpkm \
...
#Create POS file.
echo -e PANEL'\t'WGT'\t'ID'\t'CHR'\t'P0'\t'P1'\t'N >$analysis/skin/eczema.P01.pos
cd $analysis/skin/eczema
for a in *RDat
do
my_gene=$(ls $a | cut -d"_" -f4 | sed 's/.weights.wgt.RDat//')
#Look up Ensembl ID
my_g=$(grep -w $my_gene $twinsuk/skin/ensemble_ids_unique_ensembl_names | cut -f2)
...
echo -e eczema'\t'eczema/$a'\t'$my_gene'\t'$my_chrom'\t'$my_start'\t'$my_end'\t'86 >>$analysis/skin/eczema.P01.pos
done
(head -1 $analysis/skin/eczema.P01.pos; tail -n +2 $analysis/skin/eczema.P01.pos | sort | uniq) >$analysis/skin/temp
mv $analysis/skin/temp $analysis/skin/eczema.P01.pos

echo -e PANEL'\t'WGT'\t'ID'\t'CHR'\t'P0'\t'P1'\t'N >$analysis/skin/noneczema.P01.pos
cd $analysis/skin/noneczema
for a in *RDat
do
my_gene=$(ls $a | cut -d"_" -f4 | sed 's/.weights.wgt.RDat//')
#Look up Ensembl ID
...
my_start=$(echo $my_positions | cut -d" " -f3)
my_end=$(echo $my_positions | cut -d" " -f4)
echo -e noneczema'\t'noneczema/$a'\t'$my_gene'\t'$my_chrom'\t'$my_start'\t'$my_end'\t'530 >>$analysis/skin/noneczema.P01.pos
done
(head -1 $analysis/skin/noneczema.P01.pos; tail -n +2 $analysis/skin/noneczema.P01.pos | sort | uniq) >$analysis/skin/temp
mv $analysis/skin/temp $analysis/skin/noneczema.P01.pos

echo -e PANEL'\t'WGT'\t'ID'\t'CHR'\t'P0'\t'P1'\t'N >$analysis/skin/all.P01.pos
cd $analysis/skin/all
for a in *RDat
do
my_gene=$(ls $a | cut -d"_" -f4 | sed 's/.weights.wgt.RDat//')
#Look up Ensembl ID
...
my_start=$(echo $my_positions | cut -d" " -f3)
my_end=$(echo $my_positions | cut -d" " -f4)
echo -e all'\t'all/$a'\t'$my_gene'\t'$my_chrom'\t'$my_start'\t'$my_end'\t'672 >>$analysis/skin/all.P01.pos
done
(head -1 $analysis/skin/all.P01.pos; tail -n +2 $analysis/skin/all.P01.pos | sort | uniq) >$analysis/skin/temp
mv $analysis/skin/temp $analysis/skin/all.P01.pos


cd $analysis/skin
#Process GWAS results by chromosome.
...
for chr in {1..22}
do
qsub -v coloc_threshold=0.05,gwasn=103066,tissue=$i,chrom=$chr $scripts/sub_run_twas_coloc2.sh
done
done
########################################################################################
##Results harvesting
#Collect significant results into one file (pre-conditioning)
head -1 $analysis/skin/all_out/results.euro.pval.twas.chr1.top >twinsuk_skin.all
for i in "${my_tissue[@]}"
do
for a in $analysis/skin/${i}_out/*top
do
my_lines=$(wc -l $a | cut -d" " -f1)
if [ $my_lines -gt 1 ]
then
tail -n +2 $a >>twinsuk_skin.all
fi
done
done
#Collect significant results into one file (after-conditioning)
head -1 $analysis/skin/all_out/results.euro.pval.twas.chr2.analysis.joint_included.dat >twinsuk_skin.independent
for i in "${my_tissue[@]}"
do
for a in $analysis/skin/${i}_out/*included.dat
do
tail -n +2 $a >>twinsuk_skin.independent
done
done

#Collect significant coloc results
head -1 $analysis/skin/all_out/results.euro.pval.coloc.chr1 >twinsuk_skin.coloc.sig0.45
for i in "${my_tissue[@]}"
do
for a in $analysis/skin/${i}_out/*coloc*
do
cat $a | awk '$25 != "NA" {print $0}' | awk -v OFS="\t" '$25 > 0.45 {print $0}' | tail -n +2 >>twinsuk_skin.coloc.sig0.45
done
done
