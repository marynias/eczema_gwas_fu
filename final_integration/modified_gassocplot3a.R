...
#' @export
assoc_plot3a <- function(data, corr=NULL, corr.top=NULL, ylab=NULL, title=NULL, subtitle=NULL, type="log10p", x.min=NULL, x.max=NULL, top.marker=NULL, legend=TRUE, my_locus_lab=NULL, my_locus_pos=NULL){
  ...
  marker.plot <- plot_assoc3a(data, corr, corr.top, x.min, x.max, top.marker, ylab, type, my_locus_lab, my_locus_pos)
  
  # Combined plot
  legend <- FALSE
  combined.plot <- plot_assoc_combined3a(recombination.plot, gene.plot, marker.plot, title, subtitle, ngenes, legend)
...
#' plot_assoc_combined3a
#'
#' plot_assoc_combineda combines the recombination plot, gene bar and association scatter graph  
...
#' @author James R Staley <james.staley@bristol.ac.uk>
#' @export
plot_assoc_combined3a <- function(recombination.plot, gene.plot, marker.plot, title=NULL, subtitle=NULL, ngenes, r2_legend=FALSE){
...
#' plot_assoc3a
#'
#' plot_assoc3a plots a scatter graph of associations (e.g. log10 p-values) 
...
#' @author James R Staley <james.staley@bristol.ac.uk>
#' @export
plot_assoc3a <- function(data, corr=NULL, corr.top=NULL, x.min, x.max, top.marker=NULL, ylab, type="log10p", my_locus_lab=NULL, my_locus_pos=NULL){
  ...
  ylim <- max(data$stats)
  #Define index SNP.
  my_locus <- data.frame(marker=my_locus_lab, chr=lead_marker$chr, pos=my_locus_pos, stats=data[data$marker==my_locus_lab,]$stats)
  marker.plot <- ggplot(aes(x=pos, y=stats), data=data) + geom_point(aes(fill=r2), pch=21, size=3.5) + scale_fill_manual(values=c("#DCDCDC", "#66FFFF", "#66FF66", "#FFCC00", "#FF9933", "#CC3300", "#FF0000"), drop=FALSE) + geom_point(data=lead_marker, aes(pos,stats), pch=22, colour="black", fill="black", size=4.2) + geom_point(data=my_locus, aes(pos,stats), pch=23, colour="black", fill="purple", size=4.2) + theme_bw() + ylab(ylab) + xlab(NULL) + scale_y_continuous(limits=c(0,ylim)) + theme(axis.title.y=element_text(vjust=2.25, size=16), axis.text=element_text(size=14)) + theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank()) + scale_x_continuous(limits=c(x.min,x.max), breaks=NULL) + theme(axis.title=element_text(size=10)) + theme(legend.text=element_text(size=11), legend.title=element_text(size=12), legend.background = element_rect(colour = "black")) + theme(panel.background=element_rect(fill=NA)) + theme(legend.position="bottom") + guides(fill = guide_legend(nrow = 1)) 
