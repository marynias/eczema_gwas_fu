...
#' assoc_plot4a
#'
#' assoc_plot4a plots a scatter graph of associations (e.g. log10 p-values)
...
#' @param my_locus_lab plot index SNP - label
#' @param my_locus_pos plot index SNP - position
#' @import ggplot2 grid gridExtra gtable
#' @author James R Staley <james.staley@bristol.ac.uk>
#' @export
assoc_plot4a <- function(data, corr=NULL, corr.top=NULL, ylab=NULL, title=NULL, subtitle=NULL, type="log10p", x.min=NULL, x.max=NULL, top.marker=NULL, legend=TRUE, my_locus_lab=NULL, my_locus_pos=NULL){
  ...
  marker.plot <- plot_assoc4a(data, corr, corr.top, x.min, x.max, top.marker, ylab, type, my_locus_lab, my_locus_pos)
  
  # Combined plot
  combined.plot <- plot_assoc_combined4a(recombination.plot, gene.plot, marker.plot, title, subtitle, ngenes, legend)
...
#' plot_assoc4a
#'
#' plot_assoc4a plots a scatter graph of associations (e.g. log10 p-values) 
...
#' @param ylab the y-axis label
#' @param type the type of the plot either log10p or probabilities
#' @param my_locus_lab plot index SNP - lab
#' @param my_locus_pos plot index SNP - pos
#' @import ggplot2
#' @author James R Staley <james.staley@bristol.ac.uk>
#' @export
plot_assoc4a  <- function(data, corr=NULL, corr.top=NULL, x.min, x.max, top.marker=NULL, ylab, type="log10p", my_locus_lab=NULL, my_locus_pos=NULL) {
  ...
  ylim <- max(data$stats)
  geomtext <- T
  my_locus <- data.frame(marker=data[which.max(data$stats),]$marker, chr=data[which.max(data$stats),]$chr, pos=data[which.max(data$stats),]$pos, stats=data[which.max(data$stats),]$stats)
  marker.plot <- ggplot(aes(x=pos, y=stats), data=data) + geom_point(aes(fill=r2), pch=21, size=3.5) + scale_fill_manual(values=c("#DCDCDC", "#66FFFF", "#66FF66", "#FFCC00", "#FF9933", "#CC3300", "#FF0000"), drop=FALSE) + geom_point(data=lead_marker, aes(pos,stats), pch=23, colour="purple", fill="purple", size=4.2) + geom_point(data=my_locus, aes(pos,stats), pch=22, colour="black", fill="black", size=1.7)  + theme_bw() + ylab(ylab) + xlab(NULL) + scale_y_continuous(limits=c(0,ylim)) + theme(axis.title.y=element_text(vjust=2.25, size=16), axis.text=element_text(size=14)) + theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank()) + scale_x_continuous(limits=c(x.min,x.max), breaks=NULL) + theme(axis.title=element_text(size=10)) + theme(legend.text=element_text(size=11), legend.title=element_text(size=12), legend.background = element_rect(colour = "black")) + theme(panel.background=element_rect(fill=NA)) + theme(legend.position="bottom") + guides(fill = guide_legend(nrow = 1))
  if(geomtext){marker.plot <- marker.plot + geom_text(data=my_locus, aes(x=pos,y=stats,label=marker), vjust=-0.85, hjust=0.5, size=3.3)}else{if(lead_marker$stats[1]/ylim>=0.3){marker.plot <- marker.plot + geom_label(data=lead_marker, aes(x=label_pos,y=stats,label=marker), label.r=unit(0, "lines"), nudge_y=(-0.05*ylim), size=4.5, alpha=1)}else{marker.plot <- marker.plot + geom_label(data=lead_marker, aes(x=label_pos,y=stats,label=marker), label.r=unit(0, "lines"), nudge_y=(0.05*ylim), size=4.5, alpha=1)}}
  if(type=="prob"){suppressMessages(marker.plot <- marker.plot + scale_y_continuous(expand = c(.1, .1),limits=c(0,ylim), breaks=pretty(data$stats, n = 10)))}
...
#' plot_assoc_combined4a
#'
#' plot_assoc_combined4a combines the recombination plot, gene bar and association scatter graph  
...
#' @author James R Staley <james.staley@bristol.ac.uk>
#' @export
plot_assoc_combined4a <- function(recombination.plot, gene.plot, marker.plot, title=NULL, subtitle=NULL, ngenes, r2_legend=TRUE){
