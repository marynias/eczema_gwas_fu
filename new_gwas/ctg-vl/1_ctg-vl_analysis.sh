...
cat $gwas/results.${gwas_name}_filtered.txt | sed 's/POS/BP/' | sed 's/EFFECT_ALLELE/A1/' | sed 's/NON_EFFECT_ALLELE/A2/' | sed 's/EFFECT_ALLELE_FREQ/FREQ/' | sed 's/PVAL/P/' | sed 's/RSID/SNP/' | awk -v OFS="\t" '{print $2, $3, $12, $4, $5, $7, $8, $9, $11, $6}' >results.${gwas_name}.ctg
...
cat $gwas/results.${gwas_name}_filtered.txt | sed 's/POS/BP/' | sed 's/EFFECT_ALLELE/A1/' | sed 's/NON_EFFECT_ALLELE/A2/' | sed 's/EFFECT_ALLELE_FREQ/FREQ/' | sed 's/PVAL/P/' | sed 's/RSID/SNP/' | awk -v OFS="\t" '($2 == 6 && $3 > 3344835 || $3 < 2847779) || ($2 != 6) {print $2, $3, $12, $4, $5, $7, $8, $9, $11, $6}' >results.${gwas_name}.nomhc.ctg
