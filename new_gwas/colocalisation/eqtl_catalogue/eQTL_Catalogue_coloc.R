...
tabix_paths_0 = read.delim("https://raw.githubusercontent.com/eQTL-Catalogue/eQTL-Catalogue-resources/master/tabix/tabix_ftp_paths.tsv", sep = "\t", header = TRUE, stringsAsFactors = FALSE) %>% dplyr::as_tibble()
imported_tabix_paths_0 = read.delim("https://raw.githubusercontent.com/eQTL-Catalogue/eQTL-Catalogue-resources/master/tabix/tabix_ftp_paths_imported.tsv", sep = "\t", header = TRUE, stringsAsFactors = FALSE) %>% dplyr::as_tibble()
...
tabix_paths <- tabix_paths %>% filter (quant_method == "ge")
#Merge the two tables
tabix_paths_all <- bind_rows(tabix_paths, imported_tabix_paths)

#Extract column names from first file
column_names = colnames(readr::read_tsv(tabix_paths_all[1,]$ftp_path, n_max = 1))
