...
missing_runs <- data.frame(rsid = character(),
                           ...
                           transcript_ensembl_id = character(),
                           hugo_name = character(),
                           study = character(),
...
imported_tabix_paths <- tabix_paths_0 %>% filter(study == "GTEx" & (qtl_group == "fibroblast" |
qtl_group == "LCL" | qtl_group == "colon_transverse" | qtl_group == "colon_sigmoid" |
qtl_group == "esophagus_mucosa" | qtl_group == "lung" | qtl_group ==  "skin_not_sun_exposed" |
qtl_group == "skin_sun_exposed" | qtl_group == "small_intestine" |
qtl_group == "spleen" | qtl_group == "blood"))
#Not interested in txtrev
tabix_paths <- tabix_paths %>% filter (quant_method == "tx" )
...
#Extract the names of transcripts
print (my_gene)
first_table <- import_eQTLCatalogue_ver_tabix(tabix_paths_all[1,]$ftp_path, coordinates, selected_gene_id = my_gene, column_names)
my_transcripts <- unique(first_table$molecular_trait_id)
#Skip to the next gene if no transcripts found
if (length(my_transcripts) == 0) {
  print("Skipping gene")
  next
  }
...
for (my_transcript in my_transcripts) {
summary_stats <- import_eQTLCatalogue_ver_tabix_tx(row$ftp_path, coordinates, selected_gene_id = my_transcript, column_names)
...
if (is.null(summary_stats))
  ...
  missing_runs[nrow(missing_runs) + 1,] = c(my_rsid, my_gene, my_transcript, my_hugo_name, row$study, row$tissue_label, row$condition_label, row$quant_method)
  next
  }
#Check if no rows for gene found
if (dim(summary_stats)[1] == 0)
{
  print("Skipping transcript, no SNPs")
  next
}
...
coloc_res <- coloc_res %>% tibble::add_column("hugo_name" = my_hugo_name, "gene_id" = my_gene, 
                                              "transcript_id" = my_transcript, "rsid" = my_rsid,
                                              "study" = row$study, "tissue" = row$tissue_label, 
...
#Save df to file
output_file <- paste(my_rsid, "_transcript_txcoloc.txt", sep="")
output_file2 <- paste(my_rsid, "_transcript_missing_runs.txt", sep="")
